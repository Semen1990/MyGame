...
const SPEED = 300.0
const SLIDE_SPEED = 370.0
const SLIDE_DURATION = 0.3
const ATTACK_DURATION = 0.5  # Длительность атаки в секундах
const ATTACK2_WINDOW = 0.1  # Промежуток времени для второй атаки после первой
...
func _physics_process(delta):
	...
	if is_attacking:
		...
		if attack_timer <= -0.1:
			is_attacking = false
			attack2_window_timer = ATTACK2_WINDOW

	# Handle attack2 window
	if attack2_window_timer > 0:
		attack2_window_timer -= delta
		if Input.is_action_just_pressed("attack") and attack2_window_timer > 0:
	...
	if Input.is_action_just_pressed("slide") and is_on_floor() and not is_attacking and not is_blocking and not is_sliding:
		...
		slide_timer = SLIDE_DURATION
		animPlayer.play("slide")
		velocity.x = SLIDE_SPEED * (1 if $AnimatedSprite2D.flip_h == false else -1)  # Направление скольжения
